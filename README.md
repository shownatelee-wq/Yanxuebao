...
本仓库承载研学宝 V1 的多端研发，当前已包含设备端 Demo 与多端 H5 原型。导师端、家长端、专家端和运营后台当前都按纯前端本地 mock / local store 方案运行，主要用于和业务部门对齐产品流程、页面结构与交互；`apps/api` 暂时保留在仓库中，但不是这几个 Web 演示端的运行前置。
...
- `apps/api`：NestJS API 服务（当前保留在仓库，非 Web 演示端运行前置）
- `apps/admin-web`：运营后台
- `apps/tutor-web`：导师端
- `apps/parent-web`：家长端
- `apps/expert-web`：专家端
...
- 运营后台纯前端展示版：登录、经营看板、机构、导师、任务库、题库、库存与多模块台账
- 导师端首批正式能力：登录、团队、小组、任务、评分、报告
- 导师端 H5 重构版：移动端工作台、团队总控页、团队级管理页、任务作品查看、本地 mock 可操作原型
- 家长端纯前端展示版：登录、学员管理、设备绑定、能力成长、家庭任务、AI 记录与设备演示
- 专家端纯前端展示版：登录、工作台、课程、知识库、挑战、资讯、智能体
...
- 运营后台：`operator_console / Yanxuebao@2026`
- 城市维护：`city_nanshan / Yanxuebao@2026`
