...
const PRODUCT_TYPE_META: Record<ProductType, { label: string; short: string }> = {
  ...
  face_to_face: { label: '线下课程', short: '线下' },
};
...
type CourseQuickEditDraft = Pick<ExpertProduct, 'pricingType' | 'price' | 'capacity' | 'schedule' | 'location' | 'bookingDeadline' | 'deliveryPlan' | 'materialFileName'>;
...
function buildEmptyChapter(productType: ProductType, index: number): CourseChapter {
  ...
  const titlePrefix = normalizedType === 'online_course' ? '第' : normalizedType === 'live_course' ? '直播环节' : normalizedType === 'offline_course' ? '课程环节' : '活动环节';
  return {
    id: createClientId('chapter'),
    title: normalizedType === 'online_course' ? `${titlePrefix} ${index + 1} 课` : `${titlePrefix} ${index + 1}`,
    duration: normalizedType === 'activity' || normalizedType === 'offline_course' ? '30分钟' : '20分钟',
    summary: '',
    contentType,
    contentUrl: '',
    fileName: '',
...
function buildProductInput(productType: ProductType): ProductWizardDraft {
  ...
  const isOfflineLike = normalizedType === 'offline_course' || normalizedType === 'activity';
  return {
    ...
    location: normalizedType === 'online_course' || normalizedType === 'live_course' ? '线上学习' : normalizedType === 'offline_course' ? '待定线下地点' : '待定活动地点',
    schedule: normalizedType === 'live_course' ? '2026-06-08 19:30' : '待安排',
    bookingDeadline: '待设置',
    deliveryPlan: isOfflineLike ? '报名预约、现场签到、专家导入、互动体验、成果反馈' : '课程学习、互动答疑、成果任务',
    chapters: [],
    liveQrCode: '',
    tags: [],
    coverFileName: '',
    ...
    courseFormat: normalizedType === 'online_course' ? '录播课 + 线上答疑' : normalizedType === 'live_course' ? '直播分享 + 互动问答' : normalizedType === 'offline_course' ? '线下课程 + 现场核销' : '预约活动 + 现场核销',
...
function previewMediaSrc(chapter: CourseChapter) {
  if (chapter.contentType === 'audio') {
    return chapter.contentUrl?.startsWith('http') ? chapter.contentUrl : 'https://interactive-examples.mdn.mozilla.net/media/cc0-audio/t-rex-roar.mp3';
  }
  return chapter.contentUrl?.startsWith('http') ? chapter.contentUrl : 'https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4';
}

function CourseChapterPreview({ chapter }: { chapter: CourseChapter }) {
  if (chapter.contentType === 'video') {
    return (
      <div className="expert-preview-player">
        <video controls playsInline preload="metadata" src={previewMediaSrc(chapter)} />
        <small>{chapter.contentUrl || chapter.fileName || '模拟视频资源'}</small>
      </div>
    );
  }
  ...
  if (chapter.contentType === 'audio') {
    return (
      <div className="expert-preview-player">
        <audio controls preload="metadata" src={previewMediaSrc(chapter)} />
        <small>{chapter.contentUrl || chapter.fileName || '模拟音频资源'}</small>
      </div>
    );
  }
  ...
  if (chapter.contentType === 'link') {
    return (
      <div className="expert-preview-file">
        <FileTextOutlined />
        <strong>{chapter.title}</strong>
        <span>{chapter.contentUrl || '待配置课程链接'}</span>
        {chapter.contentUrl ? <Button block href={chapter.contentUrl} target="_blank">打开预览链接</Button> : null}
      </div>
    );
  }
  ...
  return (
    <div className="expert-preview-file">
      <FileTextOutlined />
      <strong>{chapter.fileName || chapter.contentUrl || `${chapter.title}.pdf`}</strong>
      <span>{chapter.summary || '课程资料预览'}</span>
    </div>
  );
}

function CoursePreviewPanel({ product, chapters }: { product: ExpertProduct; chapters: CourseChapter[] }) {
  const productType = normalizedProductType(product.productType);
  return (
    <div className="expert-form-stack">
      <div className="expert-confirm-list">
        <div><span>类型</span><strong>{PRODUCT_TYPE_META[productType].label}</strong></div>
        <div><span>排期</span><strong>{product.schedule}</strong></div>
        <div><span>地点</span><strong>{product.location}</strong></div>
        <div><span>资料</span><strong>{product.materialFileName || '待配置'}</strong></div>
      </div>
      <p className="expert-note-box">{product.deliveryPlan}</p>
      <div className="expert-list">
        {chapters.map((chapter, index) => (
          <section className="expert-preview-chapter" key={chapter.id ?? `${chapter.title}_${index}`}>
            <div className="expert-inline expert-space-between">
              <strong>{String(index + 1).padStart(2, '0')} {chapter.title}</strong>
              <Tag>{courseContentTypeLabel(chapter.contentType)}</Tag>
            </div>
            <p>{chapter.summary}</p>
            <CourseChapterPreview chapter={chapter} />
          </section>
        ))}
        {!chapters.length ? <MobileEmpty text="暂无课程目录，请先新增章节。" /> : null}
      </div>
    </div>
  );
}
...
export function ExpertCourseCreatePage() {
  ...
  function save() {
    ...
    const deliveryPlan = [
      ...
      input.productType === 'activity' || input.productType === 'offline_course' ? `集合信息：${input.meetingPoint}；安全须知：${input.safetyNotice}` : '',
      ...
      {step === 4 ? (
        <SectionCard title="交付与章节" note="章节进入独立二级页维护，列表只展示目录和顺序">
          <div className="expert-form-stack">
            ...
            {input.productType === 'activity' || input.productType === 'offline_course' ? (
              <>
...
export function ExpertCourseDetailPage() {
  ...
  const { state, saveProduct, setProductStatus } = useExpertStore();
  const [messageApi, contextHolder] = message.useMessage();
  const [activeTab, setActiveTab] = useState<'intro' | 'catalog'>('intro');
  const [previewOpen, setPreviewOpen] = useState(false);
  const [quickEditOpen, setQuickEditOpen] = useState(false);
  const [quickEditDraft, setQuickEditDraft] = useState<CourseQuickEditDraft | null>(null);
  ...
  const visibleTags = course.tags.filter((tag, index, list) => tag !== PRODUCT_TYPE_META[productType].label && list.indexOf(tag) === index);
  const productOrders = state.orders.filter((order) => order.productId === course.id);
  const orderAmount = productOrders.reduce((sum, order) => sum + order.amount - (order.refundAmount ?? 0), 0);
  const productRefunds = state.refundRequests.filter((refund) => refund.productId === course.id);
  const previewIssues = getCoursePreviewIssues(productToInput(course, chapters));
  ...
  function openQuickEdit() {
    setQuickEditDraft({
      ...
      deliveryPlan: course.deliveryPlan,
      materialFileName: course.materialFileName ?? '',
    });
    setQuickEditOpen(true);
  }

  function updateQuickEdit<Key extends keyof CourseQuickEditDraft>(key: Key, value: CourseQuickEditDraft[Key]) {
    setQuickEditDraft((current) => (current ? { ...current, [key]: value } : current));
  }

  function submitQuickEdit() {
    if (!quickEditDraft) return;
    if (quickEditDraft.capacity <= 0) {
      messageApi.warning('库存必须大于 0');
      return;
    }
    if (quickEditDraft.pricingType === 'paid' && quickEditDraft.price <= 0) {
      messageApi.warning('付费课程需设置价格');
      return;
    }
    saveProduct({
      ...productToInput(course, chapters),
      pricingType: quickEditDraft.pricingType,
      price: quickEditDraft.pricingType === 'free' ? 0 : quickEditDraft.price,
      capacity: quickEditDraft.capacity,
      schedule: quickEditDraft.schedule,
      location: quickEditDraft.location,
      bookingDeadline: quickEditDraft.bookingDeadline,
      deliveryPlan: quickEditDraft.deliveryPlan,
      materialFileName: quickEditDraft.materialFileName,
    }, course.id);
    messageApi.success('课程价格、库存和排期已保存');
  ...
  return (
    <div className="expert-page expert-page-with-action-bar">
        ...
        <div className="expert-course-hero-body">
          <div className="expert-inline expert-space-between">
            ...
            <div className="expert-inline expert-wrap">
              <Button size="small" onClick={() => setPreviewOpen(true)}>课程预览</Button>
              <Button size="small" onClick={openQuickEdit}>快捷编辑</Button>
      ...
      <Drawer title="课程预览" placement="bottom" height="88vh" open={previewOpen} onClose={() => setPreviewOpen(false)}>
        <CoursePreviewPanel product={product} chapters={chapters} />
      </Drawer>

      <Drawer title="快捷编辑" placement="bottom" height="70vh" open={quickEditOpen} onClose={() => setQuickEditOpen(false)}>
        {quickEditDraft ? (
          <div className="expert-form-stack">
            <Segmented block value={quickEditDraft.pricingType} onChange={(value) => updateQuickEdit('pricingType', value as CourseQuickEditDraft['pricingType'])} options={[{ label: '免费', value: 'free' }, { label: '付费', value: 'paid' }]} />
            <div className="expert-form-grid">
              <label>价格<InputNumber disabled={quickEditDraft.pricingType === 'free'} min={0} value={quickEditDraft.pricingType === 'free' ? 0 : quickEditDraft.price} onChange={(value) => updateQuickEdit('price', Number(value ?? 0))} /></label>
              <label>库存<InputNumber min={1} value={quickEditDraft.capacity} onChange={(value) => updateQuickEdit('capacity', Number(value ?? 1))} /></label>
            </div>
            <label>排期<Input value={quickEditDraft.schedule} onChange={(event) => updateQuickEdit('schedule', event.target.value)} /></label>
            <label>地点/形式<Input value={quickEditDraft.location} onChange={(event) => updateQuickEdit('location', event.target.value)} /></label>
            <label>预约截止<Input value={quickEditDraft.bookingDeadline} onChange={(event) => updateQuickEdit('bookingDeadline', event.target.value)} /></label>
            <label>交付说明<Input.TextArea rows={4} value={quickEditDraft.deliveryPlan} onChange={(event) => updateQuickEdit('deliveryPlan', event.target.value)} /></label>
            <label>课程资料<Input value={quickEditDraft.materialFileName} onChange={(event) => updateQuickEdit('materialFileName', event.target.value)} /></label>
            <Button type="primary" block onClick={submitQuickEdit}>保存</Button>
          </div>
        ) : null}
      </Drawer>
...
export function ExpertCourseChapterDetailPage() {
  ...
  return (
    <div className="expert-page expert-flow-page">
      ...
      <Drawer title="章节预览" placement="bottom" height="70vh" open={previewOpen} onClose={() => setPreviewOpen(false)}>
        <div className="expert-form-stack">
          <div className="expert-confirm-list">
            <div><span>章节</span><strong>{draft.title}</strong></div>
            <div><span>类型</span><strong>{courseContentTypeLabel(draft.contentType)}</strong></div>
            <div><span>时长</span><strong>{draft.duration}</strong></div>
          </div>
          <CourseChapterPreview chapter={draft} />
        </div>
      </Drawer>
      <PageActionBar>
        ...
        {editing ? (
          ...
          <>
            <Button onClick={() => setPreviewOpen(true)}>预览当前章节</Button>
            <Button type="primary" onClick={() => setEditing(true)}>编辑章节</Button>
          </>
        )}
      </PageActionBar>
    </div>
      ...
      <SectionCard title="分销配置" note="按课程设置佣金比例，不占用我的页">
        <div className="expert-list">
          {state.products.map((product) => {
            ...
            return (
              <EntityCard key={product.id} title={product.title} subtitle={PRODUCT_TYPE_META[normalizedProductType(product.productType)].label} meta={<Switch checked={Boolean(plan?.enabled)} onChange={(enabled) => updateDistributionPlan(product.id, { enabled })} />}>
                <div className="expert-form-grid">
...
export function ExpertMeOrdersPage() {
  ...
  const [refundStatus, setRefundStatus] = useState<RefundStatus | 'all'>('pending');
  const [refundAction, setRefundAction] = useState<{ refundId: string; type: 'approve' | 'reject'; note: string } | null>(null);
  ...
  const filteredRefunds = state.refundRequests.filter((refund) => {
    if (filters.productId !== 'all' && refund.productId !== filters.productId) {
      return false;
    }
    if (refundStatus !== 'all' && refund.status !== refundStatus) {
      return false;
    }
    return true;
  });
  ...
  function openRefundAction(refundId: string, type: 'approve' | 'reject') {
    setRefundAction({ refundId, type, note: '' });
  }

  function submitRefundAction() {
    if (!refundAction) return;
    const note = refundAction.note.trim();
    if (!note) {
      messageApi.warning('请填写退款处理原因备注');
      return;
    }
    const refund = state.refundRequests.find((item) => item.id === refundAction.refundId);
    if (!refund) {
      messageApi.warning('退款申请不存在');
      return;
    }
    if (refundAction.type === 'approve') {
      approveRefund(refund.id, refund.amount, note);
      messageApi.success('退款已处理');
    } else {
      rejectRefund(refund.id, note);
      messageApi.warning('退款已驳回');
    }
  ...
  return (
    <div className="expert-page">
        ...
        {selectedOrder ? (
          <div className="expert-drawer-content">
            <div className="expert-confirm-list">
              ...
              <div><span>课程类型</span><strong>{selectedOrderProduct ? PRODUCT_TYPE_META[normalizedProductType(selectedOrderProduct.productType)].label : '未知'}</strong></div>
        ...
        <SectionCard title="退款申请" note="支持全额和部分退款处理">
          <div className="expert-form-stack">
            ...
            <Segmented
              block
              value={refundStatus}
              onChange={(value) => setRefundStatus(value as RefundStatus | 'all')}
              options={[
                ...
                return (
                  <EntityCard key={refund.id} title={product?.title ?? '未知产品'} subtitle={`${refund.customerName} · ${formatDate(refund.requestedAt)}`} meta={<Tag color={refund.status === 'pending' ? 'warning' : refund.status === 'approved' ? 'success' : 'error'}>{refund.status === 'pending' ? '待处理' : refund.status === 'approved' ? '已退款' : '已驳回'}</Tag>} tags={<Tag>{formatMoney(refund.amount)}</Tag>}>
                    ...
                    {refund.status === 'pending' && order ? (
                      <div className="expert-form-grid">
                        <Button size="small" type="primary" onClick={() => openRefundAction(refund.id, 'approve')}>确认退款</Button>
                        <Button size="small" danger onClick={() => openRefundAction(refund.id, 'reject')}>驳回</Button>
                      </div>
                    ) : null}
                  </EntityCard>
                );
      ...
      <Drawer title={refundAction?.type === 'approve' ? '确认退款备注' : '驳回退款备注'} placement="bottom" height="48vh" open={Boolean(refundAction)} onClose={() => setRefundAction(null)}>
        {refundAction ? (
          <div className="expert-form-stack">
            <Input.TextArea
              rows={5}
              value={refundAction.note}
              onChange={(event) => setRefundAction((current) => (current ? { ...current, note: event.target.value } : current))}
              placeholder={refundAction.type === 'approve' ? '请输入确认退款原因，例如：家长申请符合退款规则，确认原路退回。' : '请输入驳回原因，例如：课程资料已交付，不符合退款规则。'}
            />
            <Button block type="primary" danger={refundAction.type === 'reject'} onClick={submitRefundAction}>{refundAction.type === 'approve' ? '提交确认' : '提交驳回'}</Button>
          </div>
        ) : null}
      </Drawer>
...
type ChallengeWorkRequirementDraft = ChallengeWorkRequirement & {
  attachmentsText: string;
};

type ChallengeTaskDraft = Omit<ChallengeTask, 'attachments' | 'capabilityTags' | 'workRequirements'> & {
  attachmentsText: string;
  capabilityTagsText: string;
  workRequirements: ChallengeWorkRequirementDraft[];
};

function buildStoredFilesFromText(value: string, prefix: string): StoredFileMeta[] {
  return parseTags(value).map((name, index) => ({
...
function challengeTaskToDraft(task: ChallengeTask, index: number): ChallengeTaskDraft {
  return {
    id: task.id || createClientId('task'),
    title: task.title || `任务 ${index + 1}`,
    objective: task.objective,
    description: task.description,
    references: task.references,
    attachmentsText: task.attachments.map((file) => file.name).join('、'),
    capabilityTagsText: task.capabilityTags.join('、'),
    workRequirements: task.workRequirements.length
      ? task.workRequirements.map((work, workIndex) => ({
          ...work,
          id: work.id || createClientId('work'),
          title: work.title || `作品要求 ${workIndex + 1}`,
          attachmentsText: work.attachments.map((file) => file.name).join('、'),
        }))
      : [buildEmptyWorkRequirementDraft(0)],
  };
}

function buildEmptyWorkRequirementDraft(index: number): ChallengeWorkRequirementDraft {
  return {
    id: createClientId('work'),
    title: `作品要求 ${index + 1}`,
    requirement: '',
    attachments: [],
    attachmentsText: '',
...
function buildEmptyTaskDraft(index: number): ChallengeTaskDraft {
  return {
    ...
    attachmentsText: '',
    capabilityTagsText: '',
    workRequirements: [buildEmptyWorkRequirementDraft(0)],
  };
}

function buildTaskDraftFromTemplate(template: (typeof CHALLENGE_TEMPLATE_LIBRARY)[number]) {
  return {
    ...buildEmptyTaskDraft(0),
    title: template.title,
    objective: template.objective,
    description: template.description,
    references: `${template.title}任务单.pdf`,
    attachmentsText: `${template.title}任务单.pdf`,
    capabilityTagsText: template.tagsText,
    workRequirements: [
      {
        ...buildEmptyWorkRequirementDraft(0),
        requirement: template.workRequirement,
        attachmentsText: `${template.title}任务单.pdf`,
      },
    ],
  };
...
function taskDraftsToTasks(tasks: ChallengeTaskDraft[]): ChallengeTask[] {
  return tasks.map((task, index) => ({
    ...
    description: task.description,
    references: task.references,
    attachments: buildStoredFilesFromText(task.attachmentsText, task.id || `task_${index}`),
    capabilityTags: parseTags(task.capabilityTagsText),
    workRequirements: task.workRequirements.map((work, workIndex) => ({
      id: work.id || createClientId('work'),
      title: work.title || `作品要求 ${workIndex + 1}`,
      requirement: work.requirement,
      attachments: buildStoredFilesFromText(work.attachmentsText, work.id || `work_${index}_${workIndex}`),
    })),
  }));
}

function summarizeChallengeWorks(tasks: ChallengeTask[]) {
  return tasks.reduce((sum, task) => sum + task.workRequirements.length, 0);
}
        ...
        <div className="expert-list">
          {state.challenges.map((challenge) => (
            <EntityCard
              ...
              subtitle={`${challenge.difficulty} · ${challenge.targetAge || '10-18岁'} · ${challenge.tasks.length} 个任务 · ${summarizeChallengeWorks(challenge.tasks)} 个作品要求`}
              meta={statusTag(CHALLENGE_STATUS_META[challenge.status])}
              tags={<>{Array.from(new Set(challenge.tasks.flatMap((task) => task.capabilityTags))).slice(0, 3).map((tag) => <Tag key={tag}>{tag}</Tag>)}</>}
              actions={
                <>
...
export function ExpertChallengeCreatePage() {
  ...
  const [draft, setDraft] = useState(() => ({
    ...
    tasks: editingChallenge?.tasks?.length
      ? editingChallenge.tasks.map(challengeTaskToDraft)
      : [buildEmptyTaskDraft(0)],
  ...
  const steps = ['选择模板', '基础信息', '任务设置', '作品要求', '评分成长值', '预览发布'];
  ...
  function updateTask<Key extends keyof ChallengeTaskDraft>(taskId: string, key: Key, value: ChallengeTaskDraft[Key]) {
    setDraft((current) => ({
      ...current,
      tasks: current.tasks.map((task) => (task.id === taskId ? { ...task, [key]: value } : task)),
    }));
  }

  function addTask() {
    setDraft((current) => ({ ...current, tasks: [...current.tasks, buildEmptyTaskDraft(current.tasks.length)] }));
  }

  function removeTask(taskId: string) {
    setDraft((current) => {
      if (current.tasks.length <= 1) {
        messageApi.warning('至少保留 1 个任务');
        return current;
      }
  ...
  function updateWork<Key extends keyof ChallengeWorkRequirementDraft>(taskId: string, workId: string, key: Key, value: ChallengeWorkRequirementDraft[Key]) {
    setDraft((current) => ({
      ...current,
      tasks: current.tasks.map((task) =>
        task.id === taskId
          ? {
              ...task,
              workRequirements: task.workRequirements.map((work) => (work.id === workId ? { ...work, [key]: value } : work)),
            }
          : task,
      ),
    }));
  ...
  function addWork(taskId: string) {
    setDraft((current) => ({
      ...current,
      tasks: current.tasks.map((task) =>
        task.id === taskId
          ? { ...task, workRequirements: [...task.workRequirements, buildEmptyWorkRequirementDraft(task.workRequirements.length)] }
          : task,
      ),
    }));
  }

  function removeWork(taskId: string, workId: string) {
    setDraft((current) => ({
      ...current,
      tasks: current.tasks.map((task) => {
        if (task.id !== taskId) return task;
        if (task.workRequirements.length <= 1) {
          messageApi.warning('每个任务至少保留 1 个作品要求');
          return task;
        }
        return { ...task, workRequirements: task.workRequirements.filter((work) => work.id !== workId) };
      }),
    }));
  }
  ...
  function applyTemplate(template: (typeof CHALLENGE_TEMPLATE_LIBRARY)[number]) {
    setDraft((current) => ({
      ...
      tasks: [buildTaskDraftFromTemplate(template)],
    }));
    setStep(1);
  }
  ...
  function validateStep(currentStep = step) {
    ...
    if (currentStep >= 2) {
      const invalidTaskIndex = draft.tasks.findIndex((task) => !task.title.trim() || !task.objective.trim() || !task.description.trim());
      if (invalidTaskIndex >= 0) {
        messageApi.warning(`请补充第 ${invalidTaskIndex + 1} 个任务的名称、目标和描述`);
        return false;
      }
      const overflowTaskIndex = draft.tasks.findIndex((task) => parseTags(task.capabilityTagsText).length > 5);
      if (overflowTaskIndex >= 0) {
        messageApi.warning(`第 ${overflowTaskIndex + 1} 个任务最多设置 5 个能力标签`);
        return false;
      }
    }
    if (currentStep >= 3) {
      const invalidWorkTaskIndex = draft.tasks.findIndex((task) => task.workRequirements.length === 0 || task.workRequirements.some((work) => !work.requirement.trim()));
      if (invalidWorkTaskIndex >= 0) {
        messageApi.warning(`请补充第 ${invalidWorkTaskIndex + 1} 个任务的作品要求`);
        return false;
      }
    }
  ...
  function submit(status: ChallengeStatus) {
    ...
    const tasks = taskDraftsToTasks(draft.tasks);
    const firstTask = tasks[0];
    const firstWork = firstTask?.workRequirements[0];
    const tags = Array.from(new Set(tasks.flatMap((task) => task.capabilityTags)));
    ...
    const id = saveChallenge({
      ...
      workRequirement: firstWork?.requirement ?? '',
      references: firstTask?.references ?? '',
  ...
  return (
    <div className="expert-page expert-flow-page">
      ...
      {step === 2 ? (
        <SectionCard title="任务设置" note="一个挑战可以配置 1 个或多个任务，每个任务最多 5 个能力标签" extra={<Button size="small" icon={<PlusOutlined />} onClick={addTask}>新增任务</Button>}>
          <div className="expert-form-stack">
            {draft.tasks.map((task, index) => {
              const capabilityTags = parseTags(task.capabilityTagsText);
              return (
                <div className="expert-challenge-edit-block" key={task.id}>
                  <div className="expert-inline expert-space-between">
                    <strong>任务 {index + 1}</strong>
                    <Button size="small" danger disabled={draft.tasks.length <= 1} onClick={() => removeTask(task.id)}>删除</Button>
                  </div>
                  <label>任务名称<Input value={task.title} onChange={(event) => updateTask(task.id, 'title', event.target.value)} /></label>
                  <label>任务目标<Input.TextArea rows={3} value={task.objective} onChange={(event) => updateTask(task.id, 'objective', event.target.value)} /></label>
                  <label>任务描述<Input.TextArea rows={4} value={task.description} onChange={(event) => updateTask(task.id, 'description', event.target.value)} /></label>
                  <label>参考资料<Input.TextArea rows={3} value={task.references} onChange={(event) => updateTask(task.id, 'references', event.target.value)} /></label>
                  <label>附件名称<Input value={task.attachmentsText} onChange={(event) => updateTask(task.id, 'attachmentsText', event.target.value)} placeholder="任务单.pdf、观察表.xlsx" /></label>
                  <label>能力标签<Input value={task.capabilityTagsText} onChange={(event) => updateTask(task.id, 'capabilityTagsText', event.target.value)} placeholder="3D打印、空间想象" /></label>
                  <small className={capabilityTags.length > 5 ? 'expert-field-error' : ''}>{capabilityTags.length}/5 个能力标签</small>
                </div>
              );
            })}
          </div>
      ...
      {step === 3 ? (
        <SectionCard title="作品要求" note="每个任务可以配置 1 个或多个作品要求">
          <div className="expert-form-stack">
            {draft.tasks.map((task, taskIndex) => (
              <div className="expert-challenge-edit-block" key={task.id}>
                <div className="expert-inline expert-space-between">
                  <strong>任务 {taskIndex + 1}：{task.title || '未命名任务'}</strong>
                  <Button size="small" icon={<PlusOutlined />} onClick={() => addWork(task.id)}>新增作品</Button>
                </div>
                {task.workRequirements.map((work, workIndex) => (
                  <div className="expert-challenge-work-block" key={work.id}>
                    <div className="expert-inline expert-space-between">
                      <span>作品 {workIndex + 1}</span>
                      <Button size="small" danger disabled={task.workRequirements.length <= 1} onClick={() => removeWork(task.id, work.id)}>删除</Button>
                    </div>
                    <label>作品名称<Input value={work.title} onChange={(event) => updateWork(task.id, work.id, 'title', event.target.value)} /></label>
                    <label>作品要求<Input.TextArea rows={4} value={work.requirement} onChange={(event) => updateWork(task.id, work.id, 'requirement', event.target.value)} /></label>
                    <label>附件名称<Input value={work.attachmentsText} onChange={(event) => updateWork(task.id, work.id, 'attachmentsText', event.target.value)} placeholder="作品模板.pdf、示例图.png" /></label>
                  </div>
                ))}
              </div>
            ))}
      ...
      {step === 5 ? (
        <SectionCard title="预览发布" note="确认任务内容后可保存草稿或直接发布">
          <div className="expert-confirm-list">
            ...
            <div><span>任务数量</span><strong>{draft.tasks.length} 个</strong></div>
            <div><span>作品要求</span><strong>{taskDraftsToTasks(draft.tasks).reduce((sum, task) => sum + task.workRequirements.length, 0)} 个</strong></div>
          ...
          <div className="expert-list expert-top-gap">
            {taskDraftsToTasks(draft.tasks).map((task, index) => (
              <div className="expert-challenge-preview-block" key={task.id}>
                <strong>任务 {index + 1}：{task.title}</strong>
                <p>{task.objective}</p>
                <div className="expert-inline expert-wrap">{task.capabilityTags.map((tag) => <Tag key={tag}>{tag}</Tag>)}</div>
                <small>{task.workRequirements.length} 个作品要求：{task.workRequirements.map((work) => work.title).join('、')}</small>
              </div>
            ))}
          </div>
        </SectionCard>
