...
export type ChallengeWorkRequirement = {
  id: string;
  title: string;
  requirement: string;
...
export type ChallengeTask = {
  ...
  workRequirements: ChallengeWorkRequirement[];
};
...
function normalizeChallengeWorkRequirements(
  workRequirements: Partial<ChallengeWorkRequirement>[] | undefined,
  fallbackRequirement: string,
  fallbackAttachments: StoredFileMeta[],
  prefix: string,
): ChallengeWorkRequirement[] {
  const rawWorks = workRequirements?.length
    ? workRequirements
    : [
        {
          id: `${prefix}_work_1`,
  ...
  return rawWorks.map((work, index) => ({
    id: work.id ?? `${prefix}_work_${index + 1}`,
    title: work.title || `作品要求 ${index + 1}`,
    requirement: work.requirement || fallbackRequirement || '提交作品说明、过程证据和成果照片。',
    attachments: work.attachments ?? fallbackAttachments ?? [],
  }));
}

function normalizeChallengeTasks(challenge: Partial<Challenge> & { tasks?: Partial<ChallengeTask>[] }): ChallengeTask[] {
  const fallbackRequirement = challenge.workRequirement ?? '提交作品说明、过程证据和成果照片。';
  const fallbackAttachments = challenge.attachments ?? [];
  const rawTasks = challenge.tasks?.length
    ? challenge.tasks
    : [
        {
          id: `${challenge.id ?? 'challenge'}_task_1`,
          title: challenge.title ? `${challenge.title}任务` : '任务 1',
          objective: challenge.objective,
          description: challenge.description,
          references: challenge.references,
          attachments: fallbackAttachments,
          capabilityTags: challenge.tags,
          workRequirements: [
            {
              id: `${challenge.id ?? 'challenge'}_work_1`,
              title: '作品要求 1',
              requirement: fallbackRequirement,
              attachments: fallbackAttachments,
            },
  ...
  return rawTasks.map((task, index) => {
    const taskId = task.id ?? `${challenge.id ?? 'challenge'}_task_${index + 1}`;
    return {
      id: taskId,
      title: task.title || `任务 ${index + 1}`,
      objective: task.objective || challenge.objective || '围绕真实问题完成探究、建模、表达和迭代。',
      description: task.description || challenge.description || '请补充任务描述。',
      references: task.references ?? challenge.references ?? '',
      attachments: task.attachments ?? fallbackAttachments,
      capabilityTags: task.capabilityTags ?? challenge.tags ?? [],
      workRequirements: normalizeChallengeWorkRequirements(task.workRequirements, fallbackRequirement, task.attachments ?? fallbackAttachments, taskId),
    };
  });
}
...
function normalizeState(value: unknown): ExpertState {
  ...
  const challenges = (parsed.challenges ?? []).map((challenge) => ({
    ...
    tags: challenge.tags ?? [],
    tasks: normalizeChallengeTasks(challenge),
...
export function ExpertStoreProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<ExpertStoreValue>(() => {
    ...
    function saveChallenge(input: ChallengeInput, challengeId?: string) {
      ...
      const timestamp = nowIso();
      const normalizedTasks = normalizeChallengeTasks({ id, ...input });
      setState((current) => {
