...
export const EXPERT_STORAGE_KEY = 'yanxuebao_expert_session';
...
export function getStoredSession() {
  ...
  const raw = window.sessionStorage.getItem(EXPERT_STORAGE_KEY);
  return raw ? (JSON.parse(raw) as WebSession) : null;
}

export function storeSession(session: WebSession) {
  if (typeof window !== 'undefined') {
    window.sessionStorage.setItem(EXPERT_STORAGE_KEY, JSON.stringify(session));
  }
}

export function clearSession() {
  if (typeof window !== 'undefined') {
    window.sessionStorage.removeItem(EXPERT_STORAGE_KEY);
  }
}
