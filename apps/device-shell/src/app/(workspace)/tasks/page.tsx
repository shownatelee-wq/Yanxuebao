...
export default function DeviceTasksPage() {
  ...
  const getGameplayLabels = (task: (typeof tasks)[number]) =>
    Array.from(new Set(task.taskSheets.map((sheet) => sheet.gameplayKind).filter(Boolean))).map(
            ...
            {filteredTasks.map((task) => (
              <Link key={task.id} href={`/tasks/${task.id}`} className="device-card-link">
                <div className="device-mini-item watch-list-card">
                  {getGameplayLabels(task).length ? (
                    <div className="device-action-chip-row" style={{ marginBottom: 8 }}>
                      {getGameplayLabels(task).map((label) => (
                  ...
                  <div className="device-action-chip-row" style={{ marginTop: 8 }}>
                    ...
                    {getGameplayLabels(task).length ? <Tag color="gold">含 {getGameplayLabels(task).length} 类趣味玩法</Tag> : null}
